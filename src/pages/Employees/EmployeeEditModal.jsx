  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Edit Employee: ${employee?.name}`} size="lg">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          <label className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Update Pin Code (Optional)</label>
          <input {...register('pinCode')} type="text" placeholder="Leave empty to keep current PIN" className="mt-1 block w-full rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-2 text-sm text-zinc-900 dark:text-zinc-100 focus:border-zinc-900 dark:focus:border-zinc-100 focus:outline-none transition-colors" />
