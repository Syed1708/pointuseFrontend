...
export default function LeavesDashboard() {
  ...
  const { data: requests = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const endpoint = isManagerOrAdmin ? '/leaves/admin/list' : '/leaves/my-requests';
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      {/* Title */}
      <div>
        <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-50 tracking-tight">Leave & Vacation Manager</h1>
        ...
        {!isManagerOrAdmin && (
          <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-6 shadow-sm transition-colors">
            <h3 className="text-base font-semibold text-zinc-900 dark:text-zinc-50 mb-4 flex items-center">
              <FiCalendar className="mr-2 h-5 w-5 text-indigo-500" /> New Leave Request
            </h3>
            ...
            <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4">
              <div>
                ...
                <input {...register('startDate')} type="date" className="mt-1 block w-full rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-2 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:border-zinc-900" />
              ...
              <div>
                ...
                <input {...register('endDate')} type="date" className="mt-1 block w-full rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-2 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:border-zinc-900" />
                {errors.endDate && <p className="mt-1 text-xs text-red-500">{errors.endDate.message}</p>}
              </div>

              {/* 🛑 UPGRADED REASON DROPDOWN */}
              <div>
                <label className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Reason / Motif</label>
                <select {...register('reason')} className="mt-1 block w-full rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-2 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none bg-white dark:bg-zinc-900">
                  <option value="vacances">Vacances (Vacation)</option>
                  <option value="maladie">Maladie (Sickness)</option>
                  <option value="sans_solde">Congé Sans Solde (Unpaid Leave)</option>
                  <option value="personnel">Raison Personnelle (Personal)</option>
              ...
              {/* 🛑 NEW NOTE FIELD */}
              <div>
                <label className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Note (Optional)</label>
                <textarea 
                  {...register('note')} 
              ...
              <button type="submit" disabled={isSubmitting} className="w-full flex items-center justify-center rounded-lg bg-zinc-900 dark:bg-zinc-50 p-2.5 text-sm font-semibold text-white dark:text-zinc-950 hover:bg-zinc-800 dark:hover:bg-zinc-200 disabled:bg-zinc-400 transition shadow-sm">
              ...
              <table className="min-w-full divide-y divide-zinc-100 dark:divide-zinc-800 text-left text-xs">
                <thead className="bg-zinc-50 dark:bg-zinc-900 text-[10px] font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">
                  <tr>
                    ...
                    {isManagerOrAdmin && <th className="px-4 py-3 text-right">Approval Control</th>}
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                  {isLoading ? (
                    <tr><td colSpan={7} className="py-8 text-center text-zinc-400">Loading requests...</td></tr>
                  ) : requests.length === 0 ? (
                    <tr><td colSpan={7} className="py-8 text-center text-zinc-400">No requests found.</td></tr>
                  ) : (
                    requests.map((row) => {
                      ...
                      const badgeStyles = {
                        ...
                        approved: 'bg-emerald-50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-900/30',
                      ...
                      return (
                        <tr key={row._id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/10 transition">
                          {isManagerOrAdmin && (
                            <td className="px-4 py-3 flex items-center space-x-2">
                              {row.employee?.avatar ? (
                                <img src={row.employee.avatar} alt="" className="h-7 w-7 rounded-full object-cover" />
                              ) : (
                              ...
                              <span className="font-bold text-zinc-900 dark:text-zinc-50">{row.employee?.name}</span>
                            </td>
                          )}
                          <td className="px-4 py-3 text-zinc-600 dark:text-zinc-400">{formatCalendarDate(row.startDate)}</td>
                          <td className="px-4 py-3 text-zinc-600 dark:text-zinc-400">{formatCalendarDate(row.endDate)}</td>
                          <td className="px-4 py-3 text-zinc-600 dark:text-zinc-400">{row.leaveHours} hrs/day</td>
                          ...
                          <td className="px-4 py-3 space-y-1">
                            ...
                            {row.note && (
                              <p className="text-[10px] text-zinc-400 dark:text-zinc-500 italic max-w-[150px] truncate" title={row.note}>
                                "{row.note}"
                              </p>
                            )}
                          </td>
                          ...
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center rounded-md border px-2.5 py-0.5 text-xs font-semibold capitalize ${badgeStyles[row.status]}`}>
                            ...
                            <td className="px-4 py-3 text-right">
                              {isPending ? (
                                <div className="flex justify-end space-x-1.5">
                                  <button
                                    onClick={() => handleStatusMutation.mutate({ id: row._id, status: 'approved' })}
                                  ...
                                  >
                                    <FiCheck className="mr-1 h-3.5 w-3.5" /> Approve
                                  </button>
                                  <button
                                    onClick={() => handleStatusMutation.mutate({ id: row._id, status: 'rejected' })}
